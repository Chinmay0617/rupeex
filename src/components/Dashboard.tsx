...
const Dashboard: React.FC<DashboardProps> = ({ transactions, budgets, isDarkMode, baseCurrency }) => {
  const stats = useMemo(() => {
    const income = transactions
      ...
      .reduce((acc, t) => acc + convertAmount(Number(t.amount || 0), t.currency, baseCurrency), 0);
  ...
  const utilizationMetrics = useMemo(() => {
    ...
    const totalLimit = budgets.reduce((acc, b) => acc + convertAmount(Number(b.limit), b.currency, baseCurrency), 0);
    const totalSpent = budgets.reduce((acc, b) => {
      const budgetCategory = b.category.trim();
      const spentInCategory = transactions
        .filter(t =>
          (t.category?.trim() === budgetCategory) &&
          (t.type?.toUpperCase() === 'EXPENSE')
        )
        .reduce((sum, t) => sum + convertAmount(Number(t.amount), t.currency, baseCurrency), 0);
  ...
  const monthlyTrend = useMemo(() => {
    ...
    sortedTransactions.forEach(t => {
      ...
      const converted = convertAmount(Number(t.amount), t.currency, baseCurrency);

      if (!trendMap[dateKey]) trendMap[dateKey] = { income: 0, expense: 0 };

      if (t.type?.toUpperCase() === 'INCOME') trendMap[dateKey].income += converted;
      else if (t.type?.toUpperCase() === 'EXPENSE') trendMap[dateKey].expense += converted;
    });
        ...
        <div className="xl:col-span-3 glass-card rounded-6xl p-10 lg:p-14 relative overflow-hidden">
          <div className="flex flex-col relative z-10">
            <div className="space-y-10">
              ...
              <div className="max-w-3xl p-8 bg-slate-50 dark:bg-slate-900/40 rounded-[2.5rem] border border-slate-100 dark:border-slate-800/50 shadow-inner">
            ...
            {budgets.length > 0 ? (
              ...
              <div className="mt-8">
                <p className="text-[11px] font-bold text-slate-400 leading-relaxed uppercase tracking-wider">
                  No operational limits defined. Set budget caps to enable utilization metrics.
                </p>
              </div>
            )}
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
        {budgets.slice(0, 4).map((b) => {
          ...
          const spent = transactions
            ...
            .reduce((acc, t) => acc + convertAmount(Number(t.amount), t.currency, baseCurrency), 0);

          const limit = convertAmount(Number(b.limit), b.currency, baseCurrency);
